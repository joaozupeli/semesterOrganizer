...
void botaoVoltar() {
  textSize(14);
  desenhaBotao(10, 10, 80, 28, "< VOLTAR", COR_VERMELHO, COR_VERM_ESC, COR_BRANCO);
}
...
void draw() {
  ...
  if (tempoAnimacao > 0) {
    ...
    textSize(20);
    desenhaBotao(width/2 - 90, 635, 180, 45, "PULAR",
...
void desenhaInicio() {
  ...
  textSize(22);
  desenhaBotao(width / 2 - 110, 430, 220, 58, "COMEÇAR",
...
void desenhaQuiz() {
  ...
  // botao de som circular
  boolean hoverSom = mouseX > 255 && mouseX < 455 && mouseY > 85 && mouseY < 285;
  noStroke();
  fill(0, 70);
  ellipse(width / 2 + 5, 190, 230, 230);
  fill(hoverSom ? COR_AZUL_ESC : COR_AZUL);
  ellipse(width / 2, 185, 225, 225);
  image(som, 280, 110, 150, 150);



  // instrucao acima dos botoes
  fill(COR_AMARELO);
  textSize(17);
  textAlign(CENTER, CENTER);
  text("Qual letra corresponde ao som?", width / 2, 375);
...
void desenhaResultado() {
  ...
  desenhaBotao(width / 2 - 120, 472, 240, 56, "VOLTAR AO INÍCIO",
...
void mousePressed() {
  ...
  if (tempoAnimacao > 0) {
    if (mouseX > width/2 - 90 && mouseX < width/2 + 90 && mouseY > 635 && mouseY < 680) {
      tempoAnimacao = 0;
      if (resultadoQuiz.equals("acerto")) {
        videoAcerto.stop();
      } else {
  ...
  if (tela.equals("inicio")) {
    ...
    if (mouseX > 10 && mouseX < 100 && mouseY > 10 && mouseY < 45) {
      tela = "inicio";
    } else if (mouseX > 255 && mouseX < 455 && mouseY > 85 && mouseY < 285) {
