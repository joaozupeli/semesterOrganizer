...
// Wallpaper com fade escuro gradiente no topo
void desenhaFundo() {
  image(wallpaper, 0, 0, width, height);
  for (int y = 0; y < height; y++) {
    float alpha = map(y, 0, height, 150, 0);
    fill(20, 10, 40, alpha);
    noStroke();
    rect(0, y, width, 1);
  }
...
// Botao generico com hover e sombra
...
void desenhaQuiz() {
  ...
  boolean hoverSom = mouseX > 260 && mouseX < 410 && mouseY > 80 && mouseY < 300;
  noStroke();
  fill(0, 60);
  ellipse(width / 2 + 4, 194, 164, 164);
  fill(hoverSom ? COR_AZUL_ESC : COR_AZUL);
  ellipse(width / 2, 190, 160, 160);
...
void mousePressed() {
  if (tela.equals("inicio")) {
    ...
    if (mouseX > 10 && mouseX < 100 && mouseY > 10 && mouseY < 45) {
      tela = "inicio";
    } else if (mouseX > 260 && mouseX < 410 && mouseY > 80 && mouseY < 300) {
